	...
	if(show.all) {
		...
		e$all.countries.chb <- gcheckbox('All countries', checked=FALSE, cont=g2,
									handler=function(h,...){
										...
										if(prediction) enabled(parent.env$TableB.show.traj) <- !svalue(h$obj)
										})
		addSpace(g2, 15)
...
get.table.of.countries.from.meta <- function(sim.dir, prediction=FALSE, sorted=TRUE) {
	if(prediction) {
		...
		loc.data <- data.frame(code=mcmc.set$meta$regions$country_code, name=mcmc.set$meta$regions$country_name)	}
	if(sorted) {
