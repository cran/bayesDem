...
get.table.of.countries.from.prediction <- function(sim.dir, not.predicted=TRUE, sorted=TRUE, 
	...
	if (get.nr.countries(mcmc.set$meta) <= get.nrest.countries(mcmc.set$meta)) {
		gmessage("No countries/regions without a prediction available.")
		return(NULL)
	}
	loc.data.sim.extra <- loc.data.sim[(get.nrest.countries(mcmc.set$meta)+1):(get.nr.countries(mcmc.set$meta)),]
